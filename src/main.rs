...
async fn run() {
    println!("total multiplications: {}", ITERATIONS/4);
    let input = rand();

    let gpu_start = Instant::now();
    let steps = execute_gpu(&input).await.unwrap();
    println!("gpu (including shader init): {:.2?}", gpu_start.elapsed());
    let cpu_start = Instant::now();
    let mut out = Vec::new();
    for i in 0..(ITERATIONS/4) {
        let offset = 4 * i;
        let in0 = BigUint::new(input.values[offset].to_vec());
        let in1 = BigUint::new(input.values[offset + 1].to_vec());
        let in2 = BigUint::new(input.values[offset + 2].to_vec());
...
async fn execute_gpu_inner(
    ...
    let storage_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: bytemuck::cast_ref::<Input, [u8; 4*ITERATIONS*LIMB_COUNT]>(numbers),
    ...
    println!("gpu: {:.2?}", gpu_start.elapsed());
    ...
    if let Ok(Ok(())) = receiver.recv_async().await {
        ...
        let result = bytemuck::cast_ref::<[u8; 4*ITERATIONS*LIMB_COUNT], Input>(&bytes).clone();
